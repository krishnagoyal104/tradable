		...
		<div id="tradable-card" onClick={() => props.navigate(bool ? props.tokenId : props.orderId)}>
			...
			<div id="tradable-card-bottom">
				<div id="tradable-card-bottom-left">
					<span id="issuer"><b>{bool ? props.item_details['Item name'] : props.metadata.item_details['Item name']}</b></span>
					<span id="name">{bool ? props.item_details['Item type'] : props.metadata.item_details['Item type']}</span>
				</div>
				<div id="tradable-card-bottom-right">
					{!bool && <p>Ξ {props.price && fromWei(props.price)}</p>}
				</div>
			</div>
		</div>
	);
